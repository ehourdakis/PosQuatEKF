...
int main(int argc, char** argv)
{
    ...
    bool loaded = load_and_interpolate_poses("/files/Projects/UnderDev/PosQuatEKF/data/outliers.csv", poses);
    if(!loaded) {
    ...
    PoseQuaternionEKF<double> pqekf(
        poses[0].position,  // Initial position
        poses[0].orientation, // Initial orientation
    ...
    for(size_t i = 1; i <= poses.size(); i++)
    {
        ...
        if((i%15)==0){
            plot_demo(pqekf.getStates(), poses, pqekf, i, cov_draw);
        }
